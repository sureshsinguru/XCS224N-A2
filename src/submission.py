...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  grad_outside_vecs -- the gradient with respect to all the outside word vectors
   ...
   - A softmax() function provided (utils/utils.py) which takes as input a vector/matrix of values and returns the softmax 
   for each value in  the vector, relative to the others.
  """
  ...
  ### START CODE HERE
  prodvec = np.matmul(outside_vectors, center_word_vec)
  prodvecprobs = softmax(prodvec)
  loss = -np.log(prodvecprobs[outside_word_idx])
 
  prodvecprobs[outside_word_idx] = prodvecprobs[outside_word_idx] - 1   
  
  grad_center_vec = np.matmul(outside_vectors.T, prodvecprobs)

  grad_outside_vecs = np.outer(prodvecprobs, center_word_vec)
  ### END CODE HERE
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  centerWordVec = center_word_vectors[center_id]
  for word in outside_words:
        outside_words_id = word2ind[word]
        loss_current, gradc, grado =  word2vec_loss_and_gradient(center_word_vec=centerWordVec,
                                                                outside_word_idx=outside_words_id,
                                                                outside_vectors=outside_vectors,
                                                                dataset=dataset)
        loss += loss_current
        grad_center_vecs[center_id] += gradc
